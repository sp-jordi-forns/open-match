...
func run(fe pb.FrontendServiceClient) {
	log.Printf("🛠️ Creating %d tickets for this iteration", ticketsPerIter)

	for i := 1; i <= ticketsPerIter; i++ {
		...
		level := req.Ticket.SearchFields.DoubleArgs["level"]
		msg := fmt.Sprintf("🎫 Level:%f id:%s", req.Ticket.SearchFields.DoubleArgs["level"], resp.Ticket.Id)
		if 10 <= level && level <=35 {
			msg += " 👈"
		}
...
func deleteOnAssign(fe pb.FrontendServiceClient, t *pb.Ticket) {
	for {
		...
		if got.GetAssignment() != nil {
			log.Printf("👾️ %v ⟷ %v", got.GetId(), got.GetAssignment().Connection)
			break
		}
